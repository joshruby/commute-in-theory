<script>
    export let weekdaySelection = 'All';

    const weekdayGroups = ['All', 'Business', 'Weekend']
...
<div class="border rounded-xl shadow-sm">
    ...
    <div class="grid grid-cols-3 divide-x my-4">
        {#each weekdayGroups as weekday}
            <div class="text-center">
                <input
                    type="radio"
                    bind:group={weekdaySelection}
                    value={weekday}
                    id={weekday + randStr}
                    class="peer sr-only"
                >
                <label
                    for={weekday + randStr}
                    class="mx-1 md:mx-4 cursor-pointer text-slate-400 peer-hover:text-baby-blue peer-checked:font-bold peer-checked:text-baby-blue ease-in-out duration-150"
                >
                    {weekday}
                </label>
            </div>
    ...
    <div class="grid grid-cols-7 divide-x my-4">
